  ...
  export default defineComponent({
    ...
    setup(props, { emit }) {
      ...
      const updateCurrentText = () => {
        if (
          props.textBlocks.length > 0 &&
          currentBlockIndex.value < props.textBlocks.length
        ) {
        ...
        hasMoreText.value =
          currentBlockIndex.value < props.textBlocks.length - 1;
      };
      ...
      const resetState = () => {
        ...
        debug.log(
          `Dialog reset with ${
            props.textBlocks.length
          } blocks. First block: "${currentText.value.substring(0, 20)}..."`
        );
      };
...
<style lang="scss" scoped>
  ...
  .xp-dialog-box {
    ...
    box-shadow: 0 10px 30px rgba(0, 0, 0, 0.5),
      inset 0 0 20px rgba(var(--ion-color-primary-rgb), 0.1);
