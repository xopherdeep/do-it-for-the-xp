<template>
  <div class="tropical-fx" :class="`intensity-${intensity}`">
    <!-- Sunrays -->
    <div class="sunrays-container">
      <div v-for="i in 6" :key="`sunray-${i}`" 
           class="sunray" 
           :class="`sunray-${i}`"
           :style="{
             '--angle': `${(i - 3.5) * 10}deg`,
    ...
    <div class="birds-container">
      <div v-for="i in birdCount" :key="`bird-${i}`"
           class="bird"
           ref="birdRefs"
...
export default defineComponent({
  ...
  setup(props, { emit }) {
    ...
    onUnmounted(() => {
      // Clear all timers
      allTimers.value.forEach(clearTimeout);
      allTimers.value = [];
    });
...
.tropical-fx {
  ...
  @keyframes sunray-flicker {
    0%, 100% { opacity: var(--opacity, 0.6); }
    50% { opacity: calc(var(--opacity, 0.6) + 0.2); }
  }
  
  @keyframes sunray-rotate {
    0% { transform: rotate(var(--angle, 0deg)); }
    33% { transform: rotate(calc(var(--angle, 0deg) - 4deg)); }
    66% { transform: rotate(calc(var(--angle, 0deg) + 4deg)); }
  ...
  @keyframes bird-flight {
    ...
    30%, 60% {
      transform: 
        translateX(calc(var(--land-x) - 50%))
        translateY(var(--land-y))
        scale(1);
    }
    75% {
      transform: 
        translateX(calc(var(--land-x) - 30%))
        translateY(calc(var(--land-y) - 50px))
        scale(0.8);
    }
    90% {
      transform: 
        translateX(calc(var(--land-x) - 10%))
        translateY(calc(var(--land-y) - 100px))
        scale(0.5);
      opacity: 1;
    }
    100% {
