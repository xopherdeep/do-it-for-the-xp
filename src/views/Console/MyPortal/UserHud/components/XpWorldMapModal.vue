<template>
  ...
  <!-- CENTERED MODAL -->
  <div v-if="isOpen" class="centered-modal-wrapper">
    <button class="close-modal-btn" @click.stop="close">
    ...
    <ion-card class="world-map-card rpg-box bg-slide bg-slide-modal icon-colors">
      ...
      <ion-card-content class="content">
        <div class="main-layout">
            ...
            <div class="crystals-circle">
              ...
              >
                ...
                <div v-else class="locked-icon-wrapper">
                  <i class="fad fa-lg fa-egg egg-icon" :class="pegasus.id"></i>
                  <i class="fad fa-lg fa-pegasus ghost-icon" :class="pegasus.id"></i>
                </div>

                <div v-if="pegasus.obtained" class="glow-effect" :style="{ background: pegasus.color }"></div>
              </div>
              ...
              >
                ...
                <!-- IF LOCKED: SHOW EGG NORMALLY, PEGASUS ON HOVER -->
                <div v-else class="locked-icon-wrapper">
                  <i class="fad fa-lg fa-egg egg-icon" :class="pegasiList[8].id"></i>
                  <i class="fad fa-lg fa-pegasus ghost-icon" :class="pegasiList[8].id"></i>
                </div>

                <div v-if="pegasiList[8].obtained" class="glow-effect" :style="{ background: pegasiList[8].color === '#130f40' ? 'white' : pegasiList[8].color, opacity: pegasiList[8].color === '#130f40' ? '0.3' : '0.4' }"></div>
              </div>
            </div>
          </div>
          ...
          <!-- BOTTOM: INFO BOX -->
          <!-- BOTTOM: INFO BOX -->
          <div class="info-box rpg-box-inner" v-if="activePegasusData">
            <!-- Large Icon on Left -->
            <div class="large-icon-wrapper">
                ...
                ></i>
                <i v-else class="fad fa-egg" :class="activePegasusData.id" style="opacity: 0.5;"></i>
            </div>
            ...
            <div class="text-content">
                ...
                <p class="lore-text">
                {{ activePegasusData.obtained ? activePegasusData.desc : 'This sacred beast is currently imprisoned in a dragon egg. Complete tasks to rescue it.' }}
                </p>
                <div class="temple-tag" v-if="activePegasusData.obtained">
...
.world-map-card {
  ...
  flex-direction: column;
  border-radius: 8px;
  box-shadow: 0 10px 30px rgba(0, 0, 0, 0.8); /* Removed inset shadow to let pattern show */
  ...
  .card-header {
    ...
    padding: 1rem;
    font-size: 2.2rem;
    font-family: 'Apple Kid', sans-serif; /* Earthbound Font */
    letter-spacing: 5px;
...
.status-summary {
  ...
  .label { font-size: 0.9rem; letter-spacing: 1px; }
  .count { color: #ffd700; font-weight: bold; font-size: 1rem; }
}
...
.crystals-circle-container {
  ...
  width: 360px;
  height: 360px;
  margin: 0 auto 1.5rem; /* Centered horizontally */
}
