<template>
  ...
  >
      ...
      <div v-if="user && user.stats" class="profile-content">
        ...
        <div class="stats-flex">
          ...
          <div class="stats-row">
            <xp-profile-stat-box
              type="hp"
              :icon="POINT_ICONS.hp.icon"
              :label="POINT_ICONS.hp.label"
              :color="POINT_ICONS.hp.color"
              :current="user.stats?.hp?.now || 0"
              :max="user.stats?.hp?.max || 0"
              :progress="(user.stats?.hp?.now || 0) / (user.stats?.hp?.max || 1)"
              @click="openAccordion('physical')"
            />

            <xp-profile-stat-box
              type="mp"
              :icon="POINT_ICONS.mp.icon"
              :label="POINT_ICONS.mp.label"
              :color="POINT_ICONS.mp.color"
              :current="user.stats?.mp?.now || 0"
              :max="user.stats?.mp?.max || 0"
              :progress="(user.stats?.mp?.now || 0) / (user.stats?.mp?.max || 1)"
              @click="openAccordion('mental')"
            />

            <xp-profile-stat-box
              type="ap"
              :icon="POINT_ICONS.ap.icon"
              :label="POINT_ICONS.ap.label"
              :color="POINT_ICONS.ap.color"
              :current="apPercent"
              :progress="apProgress"
              :is-percentage="true"
            ...
            <xp-profile-stat-box
              type="gp"
              :icon="POINT_ICONS.gp.icon"
              :label="POINT_ICONS.gp.label"
              :color="POINT_ICONS.gp.color"
              :current="user.stats?.gp?.wallet || 0"
              :max="user.stats?.gp?.max || 1000"
              :progress="gpProgress"
              @click="openAccordion('relational')"
            />

            <xp-profile-stat-box
              type="xp"
              box-class="level-box"
              :icon="POINT_ICONS.xp.icon"
              :label="POINT_ICONS.xp.label"
              :color="POINT_ICONS.xp.color"
              :current="user.stats?.xp?.now || user.stats?.xp?.current || 0"
              :max="user.stats?.xp?.next_level || user.stats?.xp?.max || 100"
              :progress="xpProgress"
    ...
    <xp-stat-selector-modal
      :is-open="statSelector.isOpen"
      :title="statSelector.title"
      :options="statSelector.options"
      :columns="statSelector.columns"
      :fullscreen="statSelector.fullscreen"
      :height="statSelector.height"
      :selected-value="statSelector.selectedValue"
      @close="statSelector.isOpen = false"
