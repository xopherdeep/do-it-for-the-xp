  ...
  setup() {
    ...
    const splashScreens = ref<ExtendedSplashScreen[]>([
      ...
      // Credits screen with hometown image
      {
        ...
        alt: "Opening Credits",
        duration, // Extended duration for credits
        soundEffect: 'select',
    ...
    const playCreditsMusic = () => {
      ...
      try {
        ...
        if (creditsTracks && Array.isArray(creditsTracks) && creditsTracks.length > 0) {
          ...
          audio.volume = 0.6;
          audio.loop = true;
          audio.play().catch(err => console.error('Error playing credits music:', err));

          // Store audio element for cleanup
          bgmAudio.value = audio;
        }
      } catch (error) {
        console.error('Error playing credits music:', error);
      }
    };
    ...
    const playSoundEffect = (effectName: string) => {
      ...
      try {
        ...
        if (rpgSound && typeof rpgSound.play === 'function') {
          ...
          rpgSound.play().catch(err => console.error('Error playing sound effect:', err));
          return;
        }
