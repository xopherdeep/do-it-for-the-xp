<template>
  <ion-page :class="$options.name">
    <ion-header :translucent="true">
      <ion-toolbar class="icon-colors rpg-box">
        ...
        <ion-buttons slot="end">
          <XpLoading v-if="isLoading" style="width: 30px; height: 30px; margin-right: 10px;" />
          <ion-button
            @click="openSettings"
            color="rpg"
          >
      ...
      <ion-toolbar class="icon-colors">
        ...
        <ion-buttons slot="end">
          <ion-button @click="openNewProfileModal" color="medium">
            <i class="fad fa-heartbeat mr-1" />
            <span class="text-xs">Add</span>
          </ion-button>
          <ion-button @click="openFamilySettings" color="medium">
    ...
    <ion-content class="bg-slide bg-slide-modal icon-colors">
      ...
      <!-- Stats Overview -->
      <!-- Stats Overview -->
      <XpDashboardGrid :cols="4" class="w-[95%] mx-auto mt-3 h-full flex flex-col justify-center">
        <XpDashboardTile>
          <XpStatBox
            :value="stats.users || 0"
            label="Users"
