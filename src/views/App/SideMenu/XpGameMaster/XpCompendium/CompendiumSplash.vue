    ...
    <ion-header>
      <ion-toolbar class="rpg-box">
        ...
        <ion-buttons slot="end">
          <ion-button @click="resetTutorial" id="tutorial-button">
      ...
      >
        <swiper-slide v-for="(slide, index) in slides" :key="index">
          <ion-card class="max-w-8xl text-center flex flex-col items-center justify-center space-y-4 gap-2 py-4">
            ...
            <ion-card-content>
              <!-- Dynamically load dashboard components based on slide index -->
              <component :is="getDashboardComponent(index)" :stats="stats" v-if="index < slides.length - 1" />
              <ion-button
                v-if="slide.isFinal"
                @click="getStarted"
                expand="block"
...
.ion-page {
  ion-content {
    .swiper {
      width: 100%;
      height: 90%; /* Increased height now that debug info is removed */
