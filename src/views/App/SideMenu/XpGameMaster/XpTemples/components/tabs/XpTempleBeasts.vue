  ...
  <ion-page class="xp-temple-beasts">
    <ion-content class="transparent-content">
      <div class="content-wrapper">
        ...
        <ion-accordion-group v-if="templeBeastsWithDetails.length > 0" :value="activeTiers" :multiple="true" @ionChange="onAccordionChange">
          <!-- Boss Tier -->
          <ion-accordion 
          ...
          >
            <ion-item slot="header" class="tier-header">
              ...
              <ion-label>
                <h2>Bosses</h2>
                <p>{{ getBeastsByTierGroup('boss').length }} encounter{{ getBeastsByTierGroup('boss').length !== 1 ? 's' : '' }}</p>
              </ion-label>
            </ion-item>
                ...
                <ion-row>
                  ...
                  >
                    <div class="beast-card-wrapper" @click="openBeastSelector(entry)">
                      ...
                      <div class="room-badge" @click.stop="navigateToRoom(entry)">
          ...
          >
            <ion-item slot="header" class="tier-header">
              ...
              <ion-label>
                <h2>Minibosses</h2>
                <p>{{ getBeastsByTierGroup('miniboss').length }} encounter{{ getBeastsByTierGroup('miniboss').length !== 1 ? 's' : '' }}</p>
              </ion-label>
            </ion-item>
          ...
          >
            <ion-item slot="header" class="tier-header">
              ...
              <ion-label>
                <h2>Monsters</h2>
                <p>{{ getBeastsByTierGroup('monster').length }} encounter{{ getBeastsByTierGroup('monster').length !== 1 ? 's' : '' }}</p>
              </ion-label>
            </ion-item>
                ...
                <ion-row>
                  ...
                  >
                    <div class="beast-card-wrapper" @click="openBeastSelector(entry)">
                      ...
                      <div class="room-badge small" @click.stop="navigateToRoom(entry)">
                        <i class="fas fa-map-marker-alt"></i>
                        {{ entry.roomCoords }}
                      </div>
                    </div>
...
export default defineComponent({
  ...
  setup() {
    ...
    const navigateToRoom = (entry: TempleBeast) => {
      ionRouter.push(`/game-master/compendium/setup/temples/${templeId}/room/${entry.row}/${entry.col}`);
    };
...
.tier-accordion {
  ...
  &.boss-tier {
    .tier-icon { background: rgba(255, 215, 0, 0.2); }
    .tier-icon i { color: gold; }
  }

  &.miniboss-tier {
    .tier-icon { background: rgba(180, 100, 255, 0.2); }
    .tier-icon i { color: #b464ff; }
  }

  &.monster-tier {
    .tier-icon { background: rgba(var(--ion-color-danger-rgb), 0.2); }
    .tier-icon i { color: var(--ion-color-danger); }
  }
}
...
@keyframes beast-selection-pulse {
  0% { box-shadow: 0 0 0 0 rgba(var(--ion-color-primary-rgb), 0.7); transform: scale(1); }
  50% { box-shadow: 0 0 20px 10px rgba(var(--ion-color-primary-rgb), 0); transform: scale(1.05); }
  100% { box-shadow: 0 0 0 0 rgba(var(--ion-color-primary-rgb), 0); transform: scale(1); }
}
