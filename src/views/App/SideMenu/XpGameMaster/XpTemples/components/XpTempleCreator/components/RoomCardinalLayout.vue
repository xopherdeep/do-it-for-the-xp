<template>
  <div class="cardinal-section">
    <div class="cardinal-grid">
      ...
      >
        <i :class="['fad', getAdjacentRoomIcon('north') === 'DYNAMIC_WALL' ? roomIcons.wall : getAdjacentRoomIcon('north')]"></i>
        <span class="adj-label" v-if="getAdjacentRoomLabel('north')">{{ getAdjacentRoomLabel('north') }}</span>
      </div>
      <div v-else class="grid-adj-n"></div>
      ...
      >
        <i :class="[getDoorIcon('north') === 'DYNAMIC_WALL' ? roomIcons.wall : getDoorIcon('north')]"></i>
      </div>
      ...
      >
        <i :class="['fad', getAdjacentRoomIcon('west') === 'DYNAMIC_WALL' ? roomIcons.wall : getAdjacentRoomIcon('west')]"></i>
        <span class="adj-label" v-if="getAdjacentRoomLabel('west')">{{ getAdjacentRoomLabel('west') }}</span>
      </div>
      <div v-else class="grid-adj-w"></div>

      <div 
        :class="['door-trigger', 'grid-door-w', 'icon-colors', getDoorClass('west')]"
        @click="handleDoorTriggerClick('west')"
      >
        <i :class="[getDoorIcon('west') === 'DYNAMIC_WALL' ? roomIcons.wall : getDoorIcon('west')]"></i>
      </div>
      ...
      >
        <i :class="[getDoorIcon('east') === 'DYNAMIC_WALL' ? roomIcons.wall : getDoorIcon('east')]"></i>
      </div>
      ...
      >
        <i :class="['fad', getAdjacentRoomIcon('east') === 'DYNAMIC_WALL' ? roomIcons.wall : getAdjacentRoomIcon('east')]"></i>
        <span class="adj-label" v-if="getAdjacentRoomLabel('east')">{{ getAdjacentRoomLabel('east') }}</span>
      </div>
      <div v-else class="grid-adj-e"></div>
      ...
      >
        <i :class="[getDoorIcon('south') === 'DYNAMIC_WALL' ? roomIcons.wall : getDoorIcon('south')]"></i>
      </div>
      ...
      >
        <i :class="['fad', getAdjacentRoomIcon('south') === 'DYNAMIC_WALL' ? roomIcons.wall : getAdjacentRoomIcon('south')]"></i>
        <span class="adj-label" v-if="getAdjacentRoomLabel('south')">{{ getAdjacentRoomLabel('south') }}</span>
      </div>
    </div>
  </div>
</template>
...
.center-room {
  ...
  &.type-monster, &.type-boss, &.type-miniboss { 
    color: #ff5252; 
...
.adjacent-room {
  ...
  &.grid-adj-n { grid-area: adj-n; }
  &.grid-adj-e { grid-area: adj-e; }
  &.grid-adj-s { grid-area: adj-s; }
  &.grid-adj-w { grid-area: adj-w; }
  ...
  &:hover {
    ...
    .adj-label { color: rgba(255, 255, 255, 0.7); }
  }
  
  /* Room Type Colors on hover */
  &.type-monster:hover, &.type-boss:hover, &.type-miniboss:hover, &.type-trap:hover {
  ...
  &.type-loot:hover, &.type-key:hover {
  ...
  &.type-entrance:hover, &.type-teleport:hover {
...
.door-trigger {
  ...
  &.grid-door-n { grid-area: door-n; }
  &.grid-door-e { grid-area: door-e; }
  &.grid-door-s { grid-area: door-s; }
  &.grid-door-w { grid-area: door-w; }
  ...
  &.state-wall {
    ...
    > i { color: rgba(255, 255, 255, 0.5); font-size: clamp(1rem, 4vw, 1.2rem); }
  ...
  &.state-locked {
    ...
    > i { color: var(--ion-color-warning); font-size: clamp(1.1rem, 4.5vw, 1.4rem); }
  ...
  &.state-unlocked, &.state-door {
    ...
    > i { color: var(--ion-color-success); font-size: clamp(1.1rem, 4.5vw, 1.4rem); }
  ...
  &.state-bombable {
    ...
    > i { color: var(--ion-color-secondary); font-size: clamp(1.1rem, 4.5vw, 1.4rem); }
  ...
  &.state-trapped, &.state-locked-on-enter {
...
@keyframes pulse-bombable {
  ...
  50% { transform: scale(1.15); box-shadow: 0 0 30px rgba(var(--ion-color-secondary-rgb), 0.8); }
