<template>
  <div class="achievement-schedule-tab">
    <div class="schedule-section">
      ...
      <div class="date-grid">
        <!-- Starts On Card -->
        <div class="date-card" @click="startsModalOpen = true">
          <div class="date-card__label">STARTS ON</div>
          <div class="date-card__value">
            <i class="fad fa-flag-alt mr-2"></i>
            {{ formatDate(achievement.startsOn) }}
          </div>
        </div>

        <!-- Ends On Card -->
        <div class="date-card" :class="{ 'date-card--never': !achievement.endsOn }" @click="endsModalOpen = true">
          <div class="date-card__label">ENDS ON</div>
          <div class="date-card__value">
            <i class="fad fa-flag-checkered mr-2"></i>
            {{ achievement.endsOn ? formatDate(achievement.endsOn) : 'Never' }}
          </div>
        </div>

        <!-- Due By Card -->
        <div class="date-card" @click="dueModalOpen = true">
          <div class="date-card__label">DUE TIME</div>
          <div class="date-card__value">
            <i class="fad fa-alarm-clock mr-2"></i>
            {{ formatTime(achievement.dueByTime) || 'Anytime' }}
          </div>
        </div>
      </div>
    </div>
    ...
    <div class="schedule-section">
      ...
      <div class="frequency-grid">
        <div 
          ...
          :class="{ 'frequency-card--selected': achievement.basicSchedule === type }"
          @click="updateSchedule(type)"
        >
          <div class="frequency-card__check" v-if="achievement.basicSchedule === type">
      ...
      <div class="config-details mt-4">
        <!-- Once Details -->
        <div v-if="achievement.basicSchedule === 'once'" class="config-card">
          <div class="config-card__content">
            <div class="rpg-toggle-row" @click="updateField('showDailyUntilComplete', !achievement.showDailyUntilComplete)">
        ...
        <!-- Weekly Details -->
        <div v-if="achievement.basicSchedule === 'weekly'" class="config-card">
          <div class="config-card__content">
        ...
        <!-- Custom Details -->
        <div v-if="achievement.basicSchedule === 'custom'" class="config-card">
          <div class="config-card__content">
...
const formatTime = (timeStr?: string) => {
  ...
  try {
    // If it's an ISO string from datepicker
...
.hidden-datetime-button {
  ...
  height: 100%;
  opacity: 0;
  pointer-events: none; // We trigger it manually in openModal
}
