<template>
  <ion-page :class="$options.name">
    <ion-header>
      <ion-toolbar>
        <ion-buttons slot="start">
          ...
          ></ion-back-button>
          <i class="fa fa-2x fa-place-of-worship" />
        </ion-buttons>
    ...
    <ion-content>
      <ion-note>
        <h2>You are at position: {{ currentPosition }}</h2>
        <h2>Room content: {{ currentRoom }}</h2>
      </ion-note>
          ...
          <ion-card class="maze-container icon-colors">
            ...
            >
              ...
              >
                ...
                >
                  <i :class="['fad fa-2x', ROOM_ICONS[rooms[cell].type]]"></i>
                </template>
                <template v-else>
      ...
      <ion-fab vertical="bottom" horizontal="center" class="mb-14">
        ...
        <ion-fab-list side="start" v-if="canMoveLeft" @click.stop>
          <ion-fab-button @click.stop="move('left')">
            <i class="fad fa-2x fa-arrow-left" />
          </ion-fab-button>
        </ion-fab-list>
        <ion-fab-list side="top" v-if="canMoveUp">
          <ion-fab-button
            @click.stop="
        ...
        <ion-fab-list side="end" v-if="canMoveRight">
          <ion-fab-button @click.stop="move('right')">
            <i class="fad fa-2x fa-arrow-right" />
          </ion-fab-button>
        </ion-fab-list>
        <ion-fab-list side="bottom" v-if="canMoveDown">
          <ion-fab-button @click.stop="move('down')">
          ...
          <ion-item>
            <ion-avatar slot="start">
              ...
              <ion-badge color="none">
                <i class="fad fa-key-skeleton m-0 mr-2" />
                {{ playerKeys }}
              </ion-badge>
            </ion-avatar>
