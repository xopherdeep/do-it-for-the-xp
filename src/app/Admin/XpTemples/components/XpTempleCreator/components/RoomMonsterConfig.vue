<template>
  <div class="monster-settings config-box">
        ...
        <ion-row>
          <ion-col size="4" v-for="beast in selectedBeasts" :key="beast.id">
            <div class="beast-card-wrapper">
              <XpBeastSelectorItem :name="beast.name" :avatar="beast.avatar" :bg1="beast.bg1" :bg2="beast.bg2"
    ...
    <div v-if="!isBoss" class="trap-config mt-20">
      <div class="trap-row" :class="{ active: lockOnEnter }" @click="$emit('toggle-auto-lock')">
        <div class="trap-info">
          <i class="fad fa-dungeon trap-icon" :class="{ neutral: !lockOnEnter }"></i>
          <span class="trap-label">It's a trap!</span>
        </div>
        <ion-toggle :checked="lockOnEnter" color="danger" mode="ios" @ionChange="$emit('toggle-auto-lock')"
          @click.stop></ion-toggle>
      </div>
      <p class="config-hint">
        If enabled, doors will lock upon entry until all enemies are defeated.
      </p>
    </div>
    ...
    <div v-else class="trap-config mt-20 boss-trap">
      <div class="trap-row active permanent">
        ...
        <ion-toggle :checked="true" :disabled="true" color="danger" mode="ios"></ion-toggle>
      </div>
      <p class="config-hint">
