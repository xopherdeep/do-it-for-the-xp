<template>
  <ion-page class="xp-temple-layout">
    ...
    <xp-temple-creator-header v-if="!showPreview" v-model:gridSize="gridSize" v-model:currentLevelId="currentLevelId"
      :temple-id="templeId" :temple-name="templeName" :temple-icon="templeIcon" :entrance-display="entranceDisplay"
      :show-preview="showPreview" :floors="floors" @resize="resizeGrid" @toggle-preview="showPreview = !showPreview"
      @save="saveTemple" @floor-change="setLevel" @add-floor="addFloor" @remove-floor="removeFloor" />
    <ion-content class="transparent-content" :fullscreen="true">
      <!-- Editor Mode: Show interactive grid when layout exists -->
      <div v-if="hasLayout && !isLoading" class="editor-wrapper">


        <!-- Preview Mode -->
        <div v-if="showPreview" class="tab-content w-full max-w-4xl">
          <ion-card>
            <ion-card-header>
            ...
            <ion-card-content>
              ...
              <div class="flex gap-2 mt-4">
                <ion-button expand="block" class="flex-1" @click="copyToClipboard">
                  <i class="fas fa-copy mr-2"></i>
                  Copy Code
                </ion-button>
                <ion-button expand="block" class="flex-1" color="medium" @click="showPreview = false">
        ...
        <div v-if="showMazeConfig" class="tab-content w-full max-w-4xl">
          <ion-card class="maze-config-card">
            <ion-card-header>
              <div class="flex justify-between items-center">
                ...
                <ion-button fill="clear" @click="showMazeConfig = false">
                  <i class="fas fa-times"></i>
                  Close
                </ion-button>
              </div>
            ...
            <ion-card-content>
              <div class="config-grid">
                <div v-for="room in uniqueRooms" :key="room.key" class="config-item glass-panel"
              ...
              <div v-if="uniqueRooms.length === 0" class="empty-config p-8 text-center opacity-50">
        ...
        <div v-else class="grid-wrapper">
          <xp-temple-creator-grid :maze="currentMazeSlice" :rooms-data="roomsData" :room-icons="dynamicRoomIcons"
            :side-type-info="SIDE_TYPE_INFO" :selected-cell="selectedCell" :entrance-position="entrancePosition"
            :current-level-id="currentLevelId" @cell-click="onCellClick"
            @cell-contextmenu="({ row, col, event }) => showQuickEditPopover(event, row, col)" />
        </div>
      </div>

      <!-- Empty State: Show when no layout exists -->
      <div v-else-if="!hasLayout && !isLoading" class="empty-state-wrapper">
        <div class="glass-card empty-state">
          ...
          <ion-button color="primary" @click="initializeNewLayout">
      ...
      <!-- JSON Editor Modal -->
      <XpJsonEditorModal v-if="templeData" :is-open="isJsonEditorOpen" :initial-data="templeData.temple.value"
        @close="isJsonEditorOpen = false" @apply="handleJsonApply" />
    ...
    <!-- Quick Edit Popover -->
    <xp-temple-creator-popovers v-model="quickEditState" :room-icons="ROOM_ICONS" :dungeon-items="dungeonItems"
      :has-entrance="hasEntrance" @apply-type="applyQuickType"
...
export default defineComponent({
  ...
  setup() {
    ...
    return {
      ...
      hasEntrance: creator.hasEntrance,
