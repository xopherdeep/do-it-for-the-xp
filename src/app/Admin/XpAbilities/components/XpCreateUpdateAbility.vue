  ...
  <ion-page :class="$options.name">
    <ion-header>
      <ion-toolbar class="rpg-box">
        <ion-buttons slot="start">
          <ion-back-button
            defaultHref="/game-master/compendium/abilities"
          ></ion-back-button>
        </ion-buttons>
        ...
        <ion-buttons slot="end">
          <ion-button @click="saveAbility" :color="isValid ? 'success' : 'rpg'">
            <i
    ...
    <ion-content class="bg-slide">
      <ion-card>
        <ion-card-content>
          <p>
            Create your own abilities to use in the game. Abilities can grant
            bonuses, track cooldowns, and help organize your activities.
          </p>
          <p>
            Abilities require you to maintain a certain AP threshold (not spent)
            and cost MP to cast.
          </p>
      ...
      <ion-list>
        ...
        <ion-item>
          ...
          <ion-select v-model="ability.type" interface="popover">
            <ion-select-option :value="AbilityType.RealLife"
              >Real Life Reward</ion-select-option
            >
            <ion-select-option :value="AbilityType.InGame"
              >In-Game Ability</ion-select-option
            >
          </ion-select>
          <ion-note slot="helper"
            >Real life rewards are external activities. In-game abilities are
            used within the app.</ion-note
          >
        </ion-item>
        ...
        <ion-item>
          ...
          <ion-select v-model="ability.frequency" interface="popover">
            <ion-select-option :value="TimePeriod.Hourly"
              >Hourly</ion-select-option
            >
            <ion-select-option :value="TimePeriod.Daily"
              >Daily</ion-select-option
            >
            <ion-select-option :value="TimePeriod.Weekly"
              >Weekly</ion-select-option
            >
            <ion-select-option :value="TimePeriod.Monthly"
              >Monthly</ion-select-option
            >
            <ion-select-option :value="TimePeriod.Quarterly"
              >Quarterly</ion-select-option
            >
            <ion-select-option :value="TimePeriod.BiAnnual"
              >Bi-Annual</ion-select-option
            >
            <ion-select-option :value="TimePeriod.Yearly"
              >Yearly</ion-select-option
            >
            <ion-select-option :value="TimePeriod.Flat"
        ...
        <ion-item button @click="openIconPicker">
          ...
          <div slot="end" class="icon-preview">
            <i
              v-if="ability.icon"
              :class="`${ability.iconPrefix || 'fad'} fa-${ability.icon} fa-lg`"
            ></i>
        ...
        <ion-item>
          ...
          ></ion-input>
          <ion-note slot="helper"
            >Magic Points spent when casting this ability</ion-note
          >
        </ion-item>

        <ion-item>
          <ion-label position="floating">AP Required</ion-label>
          <ion-input type="number" v-model="apCost" min="0"></ion-input>
          <ion-note slot="helper"
            >Ability Points threshold required to access this ability (not
            spent)</ion-note
          >
        </ion-item>
        ...
        <ion-item>
          ...
          <ion-select
            v-model="ability.apRequirement.period"
            interface="popover"
          >
            <ion-select-option :value="TimePeriod.Daily"
              >Daily</ion-select-option
            ...
            >
            <ion-select-option :value="TimePeriod.Flat"
              >Total (one-time)</ion-select-option
            >
          </ion-select>
          <ion-note slot="helper"
            >Time period over which AP threshold is measured</ion-note
          >
        </ion-item>
      </ion-list>
      ...
      <ion-list v-if="ability.type === AbilityType.InGame">
        ...
        <ion-item>
          ...
          >
            ...
            <ion-select-option value="constitution"
              >Constitution</ion-select-option
            >
            <ion-select-option value="intelligence"
              >Intelligence</ion-select-option
            >
            <ion-select-option value="wisdom">Wisdom</ion-select-option>
            <ion-select-option value="charisma">Charisma</ion-select-option>
          </ion-select>
          <ion-note slot="helper"
            >Attribute that enhances this ability</ion-note
          >
        </ion-item>
        ...
        <ion-item v-if="scalingAttribute !== 'none'">
          ...
          <ion-note slot="helper"
            >How much each point in the attribute increases ability
            effectiveness</ion-note
          >
        </ion-item>
      </ion-list>
    </ion-content>
  ...
  export default defineComponent({
    ...
    setup() {
      ...
      const isValid = computed(() => {
        ...
        if (
          ability.value.apRequirement.amount < 0 ||
          isNaN(ability.value.apRequirement.amount)
        ) {
          return false;
        }
        ...
        if (loadedAbility) {
          ...
          if (loadedAbility.characterRequirement?.class) {
            const className = Object.keys(
              loadedAbility.characterRequirement.class
            )[0];
            if (className) {
              selectedClass.value = className;
              selectedClassLevel.value =
                loadedAbility.characterRequirement.class[className];
            } else {
          ...
          if (loadedAbility.characterRequirement?.level) {
            characterLevelRequired.value =
              loadedAbility.characterRequirement.level;
          }
      ...
      const updateCharacterLevelRequirement = () => {
        if (characterLevelRequired.value === 0) {
          ...
          ability.value.characterRequirement.level =
            characterLevelRequired.value;
        }
      };
