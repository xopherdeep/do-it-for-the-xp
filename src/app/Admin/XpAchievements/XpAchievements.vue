<template>
  <xp-rpg-page :title="'Quests (' + achievements?.length + ')'" header-icon="fa-hand-holding-seedling">
    <template #actions>
      <!-- <ion-button @click="clickFilter">
            <i class="fad fa-filter fa-lg" />
          </ion-button> -->
      <ion-button fill="clear" :color="viewMode === 'list' ? 'primary' : 'medium'" @click="setViewMode('list')">
        <i class="fad fa-list fa-lg" />
      </ion-button>
      <ion-button fill="clear" :color="viewMode === 'grid' ? 'primary' : 'medium'" @click="setViewMode('grid')">
      ...
      <ion-popover trigger="popover-button" :dismiss-on-select="true">
        <ion-content>
          <ion-list>
            <ion-item :button="true" :detail="false" @click="showPoints = !showPoints">
    ...
    <!-- Empty State -->
    <div v-if="!hasAchievements" class="empty-state-wrapper">
      <div class="glass-card empty-state">
        <i :class="emptyStateIcon" class="empty-icon"></i>
        <h2>{{ emptyStateTitle }}</h2>
        <p>{{ emptyStateMessage }}</p>
        <ion-button v-if="groupBy === 'category' || groupBy === 'asNeeded'" fill="outline" color="rpg"
          @click="clickAdd">
    ...
    <!-- Grid View -->
    <div v-else-if="viewMode === 'grid'" class="quest-grid">
      <ion-grid>
        <ion-row>
          <ion-col size="6" size-md="4" size-lg="3" v-for="achievement in flatAchievements" :key="achievement.id">
            <div class="quest-card glass-card" :class="{ 'is-expired': new Date(achievement.endsOn) < new Date() }"
              @click="clickEdit(achievement.id)">
              <div class="quest-card-avatar">
                <!-- Beast Preview -->
                <template v-if="achievement.adventureType === 'beast'">
                  <ion-img v-if="getBeastAvatar(achievement)" :src="getBeastAvatar(achievement)" />
                  <i v-else class="fad fa-paw-claws fa-3x fallback-icon" />
                </template>

                <!-- Custom/Achievement Image -->
                <ion-img v-else-if="achievement.imageUrl" :src="achievement.imageUrl" />

                <!-- Combo Icon Fallback -->
                <i v-else :class="getAchievementTypeIcon(achievement)" class="fallback-icon" />
              </div>
              <div class="quest-card-content">
                <h3 v-html="achievement.achievementName || 'Untitled Quest'" />
    ...
    <!-- List View -->
    <div v-else class="quest-list-wrapper">
      <!-- Accordion View for By Assignee -->
      <ion-accordion-group v-if="groupBy === 'assignee'" :multiple="true">
        <ion-accordion v-for="(group, index) in groupedAchievements" :key="index" :value="`assignee-${index}`"
          class="assignee-accordion">
          <ion-item slot="header" class="assignee-header">
          ...
          <div slot="content" class="accordion-content">
            <ion-list>
              <ion-item-sliding v-for="(achievement, idx) in group.achievements" :key="idx">
                <xp-achievement-item :achievement="achievement" :categories="categories" :beasts="beasts"
                  :show-points="showPoints" :disabled="new Date(achievement.endsOn) < new Date()" />
                <ion-item-options side="start">
                  <ion-item-option color="danger" @click="clickDeleteAchievement(achievement)">
      ...
      <ion-list v-else>
        <ion-item-group v-for="(group, index) in groupedAchievements" :key="index">
          <ion-item-divider>
            <ion-label>
    ...
    <ion-footer class="quest-footer" slot="footer">
