  ...
  <ion-item :disabled="$attrs.disabled" class="quest-list-item">
    ...
    <ion-avatar slot="start" class="quest-avatar">
      ...
      <template v-if="achievement.adventureType === 'beast'">
        <ion-img v-if="beastAvatarUrl" :src="beastAvatarUrl" />
    ...
    <ion-label>
      ...
      <div class="status-icons">
        <i v-if="achievement.difficulty" class="fad" :class="difficultyIcon"
          :title="`Difficulty: ${achievement.difficulty}`" />
        <i v-if="achievement.basicSchedule" class="fad" :class="basicScheduleIcon" :title="achievement.basicSchedule" />
        <i v-if="achievement.type" class="fad" :class="achievementTypeIcon" :title="achievement.type" />
        <i v-if="achievement.requiresApproval" class="fad fa-clipboard-check" title="Requires Approval" />
        <i v-if="achievement.bonusAchievement" class="fad fa-gift" title="Bonus Quest" />
      </div>

      <!-- Quest Name with placeholder -->
      <h2 class="quest-title">
        <span v-if="achievement.achievementName" v-html="achievement.achievementName" />
...
export default defineComponent({
  props: ["achievement", "categories", "showPoints", "beasts"],
  ...
  computed: {
    ...
    comboIcon() {
      const assignment = this.achievement.type || "asNeeded";
      const depth = this.achievement.adventureType || "simple";
      const combo = LORE_COMBO_MATRIX[assignment]?.[depth] || {
        icon: "fa-scroll",
      };
      return `fad ${combo.icon} fa-lg`;
    },
    activeBeast() {
      if (!this.achievement.beastId || !this.beasts) return null;
      return this.beasts.find((b) => b.id === this.achievement.beastId);
    },
    beastAvatarUrl() {
      if (!this.activeBeast) return null;
      if (this.activeBeast.imageUrl) return this.activeBeast.imageUrl;
      if (this.activeBeast.localImage) return this.activeBeast.localImage;
      if (this.activeBeast.avatar) {
        try {
          const pad = this.activeBeast.avatar.toString().padStart(3, "0");
          return new URL(
