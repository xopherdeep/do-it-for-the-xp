<template>
  <ion-header>
    <ion-toolbar>
      <ion-back-button slot="start" @click="dismiss" defaultHref="/game-master" />
  ...
  <ion-content class="bg-slide">
    ...
    <ion-list>
      <XpAchievementItem v-for="achievement in beastlessAchievements" :achievement="achievement" :key="achievement.id">
        <template v-slot:end>
          <ion-checkbox slot="end" :checked="attached?.includes(achievement.id)"
...
import XpAchievementItem from "@/app/Admin/XpCompendium/views/XpAchievements/XpConfigAchievement/components/XpAchievementItem.vue";
