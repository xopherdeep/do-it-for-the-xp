<template>
  ...
  <div v-if="fabActive" class="menu-backdrop-overlay" @click="closeAndDeactivate"></div>

  <ion-fab ref="fabContainer" vertical="center" horizontal="center" class="icon-colors fab-page-shortcuts">
    ...
    <ion-fab-list v-for="side in sides" :key="side" :side="side">
      <ion-fab-button v-for="(button, index) in filterShortcutsBySide(side)" :key="index"
        @click="handleButtonClick(button)" class="icon-colors">
...
export default defineComponent({
  ...
  setup(props) {
    ...
    const filterShortcutsBySide = (side: string) =>
      (props.shortcuts || []).filter((s: any) => s.side == side);
