<template>
  ...
  <div v-if="fabActive" class="menu-backdrop-overlay" @click="closeMenu"></div>

  <!-- CENTERED MENU -->
  <div v-if="fabActive" class="centered-menu-wrapper">
    <xp-close-button size="lg" color="danger" class="close-menu-btn" @click="closeMenu" />
    <xp-card-menu :cardTitle="pageName" :actions="userActions" @action-click="closeMenu" />
  </div>

  <!-- PAGE MENU BUTTON -->
  <ion-fab vertical="center" horizontal="center" class="fab-page-menu" v-if="user?.stats">
    <ion-fab-button color="light" @click="toggleMenu">
      <i v-if="pageIcon != 'fort-awesome'" class="fad fa-2x" :class="`fa-${pageIcon}`" />
      <i v-else class="fab fa-lg" :class="`fa-${pageIcon}`" />
    </ion-fab-button>
  </ion-fab>
</template>
