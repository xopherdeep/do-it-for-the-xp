<template>
  <xp-portal-page :loading="!user" page-class="my-portal icon-colors">
    <!-- HUD Overlay (stats, avatar) -->
    <template #hud>
      <xp-main-hud v-if="isUserFabOn && user?.stats" :stats="user.stats">
        <template #avatar>
          <xp-fab-user-hud :user="user" :isUserFabOn="isUserFabOn" @open-profile="openUserProfileModal" />
        </template>
      </xp-main-hud>
    </template>
    ...
    <template #fabs>
      ...
      <xp-fab-page-menu v-if="isUserFabOn && fabStyle === 'retro'" :user="user" :page-name="compass.name"
        :key="`menu-${route.path}`" />

      <!-- Modern FAB (Radial Shortcuts) -->
      <xp-fab-page-shortcuts v-if="isUserFabOn && fabStyle === 'modern'" :shortcuts="userActions"
    ...
    <template #tabs>
      <ion-tabs v-if="user && user.stats">
        ...
        <!-- Docked XP Bar -->
        <div class="tab-docked-xp" v-if="user.stats && !battleState('active')" slot="bottom">
        ...
        <ion-tab-bar slot="bottom" v-if="user.stats && !battleState('active')">
          <ion-tab-button color="success" tab="my-profile" @click.prevent="openUserProfileModal">
    ...
    <template #modals>
      <!-- EQUIPMENT MODAL -->
      <xp-equipment-modal :isOpen="isRPGBoxOpen" :equipment="equipment" :user="user" @equip-item="clickItem"
        @close="closeModal" @open-pegasus-modal="openPegasusModal" />

      <!-- USER PROFILE MODAL -->
      <user-profile-modal :is-open="isUserProfileModalOpen" :user="user" @close="closeUserProfileModal" />

      <!-- PEGASUS WORLD MAP MODAL (Wind Whistle) -->
      <xp-world-map-modal :is-open="isPegasusModalOpen" @close="closePegasusModal" />
    </template>
  </xp-portal-page>
</template>
