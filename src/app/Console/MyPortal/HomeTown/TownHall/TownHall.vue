          ...
          <ion-card v-for="news in newsItems" :key="news.id">
            ...
            <ion-card-content>
              ...
              <div
                ...
                v-if="news.categories_names && news.categories_names.length"
              >
                <ion-chip
                  v-for="category in news.categories_names"
                  :key="category"
                  color="secondary"
                >
          ...
          <ion-list>
            <ion-item-group v-for="(events, date) in groupedEvents" :key="date">
              ...
              <ion-item v-for="event in events" :key="event.id">
                ...
                <ion-badge
                  :color="event.type === 'special' ? 'warning' : 'primary'"
                  slot="end"
                >
                  {{ event.type }}
                </ion-badge>
