...
import { toastController, modalController, onIonViewDidEnter } from "@ionic/vue";
import {
  IonButton,
  IonButtons,
  IonCol,
...
import "swiper/css";
import XpHpMpHud from "@/app/Console/BattleField/hud/XpHpMpHud";
...
export default defineComponent({
  ...
  computed: {
    ...
    user() {
      // Use the store's user if available, otherwise fallback to prop or empty object
      const storeUser = this.userId ? this.userStore.getUserById(this.userId) : null;
      if (storeUser) return storeUser;

      // Fallback object to prevent null access errors if store isn't ready
      return {
        name: { nick: this.userName || 'Player' },
        stats: { level: 1, hp: 100, mp: 100, gp: { wallet: 0 } }
      };
    },
    xp_achievement() {
  ...
  setup() {
    ...
    const { request, getItems, getImgObj, isLoading } = useItemFetcher();

    onIonViewDidEnter(() => {
      setActions("battle-field", []);
    });
