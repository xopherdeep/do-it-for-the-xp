          ...
          <ion-col size="12">
            <ion-card class="stats-overview">
              <ion-card-header>
                <ion-card-title class="flex items-center gap-2">
                  <xp-icon icon="heartbeat" primary="red" secondary="pink" />
                  Current Status
                </ion-card-title>
              </ion-card-header>
              <ion-card-content>
            ...
            >
              ...
              <ion-card-content>
                <ion-list>
                  <ion-item
                    v-for="(desc, statKey) in area.stats"
                    :key="statKey"
                  >
                    <ion-label>
                      <h3>
                        {{
                          statKey.toString().charAt(0).toUpperCase() +
                          statKey.toString().slice(1)
                        }}
                      </h3>
                      <p>{{ desc }}</p>
                    </ion-label>
                    <ion-badge
                      ...
                      :color="area.color"
                      :class="{
                        'stat-badge updated': changedStats.has(
                          statKey.toString()
                        ),
                      }"
                    >
...
<style lang="scss" scoped>
  .xp-view-stats {
    ion-content {
      --background: url("@/assets/images/midjourney/hospital.png") center /
        cover no-repeat;
    }
