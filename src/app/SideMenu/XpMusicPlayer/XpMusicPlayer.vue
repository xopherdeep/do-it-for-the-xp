      ...
      <div class="player-controls relative">
        <transition name="slide-fade" mode="out-in">
          <div v-if="showTrackInfo && isInfoVisible" class="track-info rpg-box">
            <div
              class="track-title rpg-box"
              :class="{ marquee: shouldMarquee }"
            >
              <span>{{ currentTrackTitle }}</span>
              <span v-if="shouldMarquee" class="marquee-duplicate">{{
                currentTrackTitle
              }}</span>
        ...
        <ion-item
          detail="false"
          lines="none"
          class="music-control-item flex flex-col"
        >
          <div class="player-buttons flex flex-row gap-4 justify-evenly">
            <ion-button
              fill="clear"
              @click="handleButtonClick($event, prevTrack)"
            ...
            <ion-button
              fill="clear"
              @click="handleButtonClick($event, rewind)"
              class="control-button"
            >
            ...
            <ion-button
              fill="clear"
              @click="handleButtonClick($event, togglePlayPause)"
            ...
            <ion-button
              fill="clear"
              @click="handleButtonClick($event, fastForward)"
              class="control-button"
            >
            ...
            <ion-button
              fill="clear"
              @click="handleButtonClick($event, nextTrack)"
              class="control-button"
            >
  ...
  export default defineComponent({
      ...
      updateTrackInfo() {
        ...
        this.$nextTick(() => {
          ...
          const titleEl = document.querySelector(
            ".xp-music-player .track-title span"
          );
          const containerEl = document.querySelector(
            ".xp-music-player .track-title"
          );

          if (titleEl && containerEl) {
            this.shouldMarquee =
              titleEl.scrollWidth > containerEl.clientWidth - 80; // Account for the BGM ON/OFF label
          }
        });
    ...
    mounted() {
      ...
      document.addEventListener(
        "audio-engine:previousTrack",
        this.handlePreviousTrack
      );
      document.addEventListener("audio-engine:nextTrack", this.handleNextTrack);
      document.addEventListener(
        "audio-engine:seekBackward",
        this.handleSeekBackward as unknown as (ev: Event) => void
      );
      document.addEventListener(
        "audio-engine:seekForward",
        this.handleSeekForward as unknown as (ev: Event) => void
      );
    },
